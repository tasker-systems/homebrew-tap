...
def main() -> int:
    ...
    base = f"https://github.com/tasker-systems/temper/releases/download/v{args.version}"
    mac_url = f"{base}/temper-v{args.version}-{MAC_TRIPLE}.tar.gz"
    linux_url = f"{base}/temper-v{args.version}-{LINUX_TRIPLE}.tar.gz"
    ...
    # The cop requires 2-space heredoc indentation; `<<~` strips exactly that
    ...
    repl = {
        ...
        "@MAC_URL@": mac_url,
        "@MAC_SHA@": args.mac_sha,
        "@LINUX_URL@": linux_url,
        "@LINUX_SHA@": args.linux_sha,
